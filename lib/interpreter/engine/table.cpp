...
Expect<void>
Interpreter::runTableCopyOp(Runtime::Instance::TableInstance &TabInst1,
                            Runtime::Instance::TableInstance &TabInst2,
                            const AST::TableInstruction &Instr) {
  /// Pop the length, source, and destination from stack.
  uint32_t Len = retrieveValue<uint32_t>(StackMgr.pop());
  uint32_t Src = retrieveValue<uint32_t>(StackMgr.pop());
  ...
  /// Replace tab_1[Dst : Dst + Len] with tab_2[Src : Src + Len].
  if (auto Refs = TabInst2.getRefs(Src, Len)) {
    if (auto Res = TabInst1.setRefs(*Refs, Dst, 0, Len)) {
      return {};
    } else {
      LOG(ERROR) << ErrInfo::InfoInstruction(Instr.getOpCode(),
                                             Instr.getOffset());
